...
import { ADD_CHECK_MARK } from '@/assets/constants';
...
function GameBoard({ socket }: { socket: WebSocket }) {
  ...
  // if (!socket) {return}
  const { gameBoard, checkedBoxes, checkedLines, sendData } = useGame(socket)
  ...
  const handleAddCheck = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (boxName) {
      sendData(ADD_CHECK_MARK, boxName); // Assuming `addCheckMark` is used to handle clicks
    }
  };
